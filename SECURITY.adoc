...
* Keycloak cryptographically protects `KC_RESTART` and `KC_AUTH_SESSION_HASH`. Their random values can resemble SQL punctuation, so `KC_RESTART` is excluded only from rules 942420 and 942440 and `KC_AUTH_SESSION_HASH` only from rule 942520 under `/auth/`.
